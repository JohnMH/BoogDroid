...
import me.albfan.boogdroid.general.BugResolutionChanges;
import me.albfan.boogdroid.general.BugStatusChanges;
import me.albfan.boogdroid.general.StatusInfo;
import me.albfan.util.Util.TaskListener;

public class Server extends me.albfan.boogdroid.general.Server {

    public Server(final String name, final String url, boolean jsonImplementation) {
        super(name, url, BUGZILLA, jsonImplementation);
    }

    public Server(final me.albfan.boogdroid.db.Server server) {
        super(server);
    }
    ...
    private void loadProductsFromIds(final String productIds) {
        final BugzillaTask task = new BugzillaTask(this, "Product.get", "'ids':" + productIds + ",'include_fields':['id', 'name', 'description']", new TaskListener() {
            List<me.albfan.boogdroid.general.Product> newList = new ArrayList<>();
